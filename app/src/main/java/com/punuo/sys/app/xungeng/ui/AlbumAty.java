...
import com.punuo.sys.app.xungeng.camera.album.view.AlbumGridView;
 ...
 */
public class AlbumAty extends MyActivity implements View.OnClickListener, AlbumGridView.OnCheckedChangeListener {
                    ...
                    public void run() {
                        for (final String path : paths) {
                            ...
                            if (path.contains("video")) {
                                ftpPath = "/" + SipInfo.devId + "/video";
                            } else {
                                ftpPath = "/" + SipInfo.devId + "/picture";
                            }
                            try {
                                myFtp.uploadSingleFile(new File(path), ftpPath, new FTP.UploadProgressListener() {
                                    @Override
                                    public void onUploadProgress(String currentStep, long uploadSize, File file) {
                                        ...
                                        } else if (currentStep.equals(FTP.FTP_CONNECT_FAIL)) {
                                            dialog.dismiss();
                                            handler.post(new Runnable() {
                                                @Override
                                                public void run() {
                                                    Toast.makeText(AlbumAty.this, "网络异常,请检查网络", Toast.LENGTH_SHORT).show();
                                                }
                                            });
                                        }
                                    }
